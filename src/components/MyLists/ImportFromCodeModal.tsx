...
export function ImportFromCodeModal({ onImport, onCancel }: {
    ...
    return (
        ...
        >
            ...
            >
                <div>
                    <h2 style={{ margin: '0 0 0.3rem', fontSize: '1.15rem', fontWeight: 'var(--font-bold)', color: 'var(--text-primary)' }}>Import from Army Code</h2>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <label style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', fontWeight: 'var(--font-semibold)' }}>Army Code(s)</label>
                        {codes.length > 1 && (
                            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--accent)', fontWeight: 'var(--font-semibold)' }}>
                    ...
                    />
                    {error && <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--error)' }}>{error}</p>}
                </div>

                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                    <button onClick={onCancel} disabled={importing} style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: '1px solid var(--border)', background: 'none', color: 'var(--text-secondary)', cursor: importing ? 'not-allowed' : 'pointer', fontSize: 'var(--text-md)', opacity: importing ? 0.5 : 1 }}>
                        Cancel
                    </button>
                    <button
                        disabled={codes.length === 0 || importing}
                        onClick={handleSubmit}
                        style={{ padding: '0.5rem 1.25rem', borderRadius: '8px', border: 'none', background: codes.length > 0 && !importing ? '#10b981' : 'var(--bg-tertiary)', color: '#fff', cursor: codes.length > 0 && !importing ? 'pointer' : 'not-allowed', fontWeight: 'var(--font-semibold)', fontSize: 'var(--text-md)', opacity: codes.length > 0 && !importing ? 1 : 0.5 }}>
