        ...
        <div className={clsx('relative w-full', className)}>
            <Listbox value={value || 0} onChange={onChange}>
                ...
                <ListboxButton className="relative w-full cursor-default rounded-xl bg-gray-800 py-4 pl-5 pr-16 text-left border border-gray-600 shadow-md focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 text-base text-gray-200 transition-colors hover:bg-gray-700 hover:border-gray-500">
                    <span className="flex items-center gap-4 min-h-[2.5rem] overflow-hidden">
                        {selectedFaction ? (
                            <>
                                <img
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="text-lg font-semibold text-white leading-tight truncate pr-2">
                        ...
                        return (
                            <div key={group.id} className="border-b border-gray-700/50 last:border-b-0">
                                ...
                                <button
                                    ...
                                    className="w-full sticky top-0 z-10 flex items-center gap-3 px-5 py-5 bg-gray-800 hover:bg-gray-700 transition-colors border-b border-gray-700/40 text-left group"
                                >
                                    {headerLogo && (
                                        <img
                                    ...
                                    <span className="flex-1 text-base font-bold text-gray-200 uppercase tracking-widest leading-none">
                                        {group.name}
                                    </span>
                                    <span className="text-xs text-gray-500 mr-1">{childCount}</span>
                                    {isCollapsed
                                ...
                                {!isCollapsed && (
                                    <div>
                                        ...
                                        {vanilla && (
                                            <ListboxOption
                                                value={vanilla.id}
                                                className="group relative cursor-default select-none py-3.5 px-5 text-gray-100 data-[focus]:bg-gray-700 data-[focus]:text-white transition-colors"
                                            >
                                                ...
                                                <span className="absolute inset-y-0 right-0 flex items-center pr-4 text-accent-500 hidden group-data-[selected]:flex">
                                        ...
                                        {sectorials.map(s => (
                                            <ListboxOption
                                                key={s.id}
                                                value={s.id}
                                                className="group relative cursor-default select-none py-3 pl-10 pr-10 text-gray-300 data-[focus]:bg-gray-700 data-[focus]:text-white transition-colors"
                                            >
                                                <div className="flex items-center gap-4">
                                                    <img
